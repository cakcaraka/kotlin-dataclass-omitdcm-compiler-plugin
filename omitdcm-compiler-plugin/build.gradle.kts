...
val omitdcmRuntime by configurations.dependencyScope("omitdcmRuntime") { isTransitive = false }

val omitdcmRuntimeClasspath =
  configurations.resolvable("omitdcmRuntimeClasspath") {
    isTransitive = false
    extendsFrom(omitdcmRuntime)
  }
...
dependencies {
  ...
  omitdcmRuntime(project(":omitdcm-annotations"))
...
buildConfig {
  ...
  packageName("com.cakcaraka.omitdcm")
  buildConfigField("String", "KOTLIN_PLUGIN_ID", "\"com.cakcaraka.omitdcm.compiler\"")
}

tasks.test {
  dependsOn(omitdcmRuntimeClasspath)
  val omitdcmRuntimeClasspath = omitdcmRuntimeClasspath.map { it.asPath }
  ...
  doFirst { systemProperty("omitdcmRuntime.classpath", omitdcmRuntimeClasspath.get()) }
...
val generateTests =
  tasks.register<JavaExec>("generateTests") {
    ...
    mainClass.set("com.cakcaraka.omitdcm.compiler.GenerateTestsKt")
    workingDir = rootDir
  }
